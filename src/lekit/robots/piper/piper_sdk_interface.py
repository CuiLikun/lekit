...
class PiperSDKInterface:
    def __init__(self, port: str = "can0", enable_timeout: float = 5.0):
        ...
        try:
            ...
            self.min_pos = [
                pos.min_angle_limit / 10.0 for pos in angel_status.all_motor_angle_limit_max_spd.motor[1:7]
            ] + [0.0]
            self.max_pos = [
                pos.max_angle_limit / 10.0 for pos in angel_status.all_motor_angle_limit_max_spd.motor[1:7]
            ] + [10.0]
        except Exception as e:
    ...
    def get_status_deg(self) -> dict[str, float]:
        ...
        try:
            ...
            logging.debug("Failed to read gripper position from SDK")
        return out
...
if __name__ == "__main__":
    # Simple test
    iface = PiperSDKInterface(port="can0", enable_timeout=5.0)
    print("Initial status:", iface.get_status_deg())
    time.sleep(1)
    # print("Setting joints to 0 deg and gripper to 5 mm")
    # iface.set_joint_positions_deg([0, 0, 0, 0, 0, 0], gripper_mm=5.0)
    # time.sleep(1)
    print("Status after move:", iface.get_status_deg())
    iface.disconnect()
